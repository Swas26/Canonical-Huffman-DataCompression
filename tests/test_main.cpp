// test_main.cpp -- main(): the built scomp binary, run as a separate process.
//
...
// user sees them. The binary is ./scomp (make test builds it first) or $SCOMP_BIN;
...
protected:
    ...
    void SetUp() override {
        ASSERT_TRUE(dir.ok());
        const char* env = std::getenv("SCOMP_BIN");
        const fs::path bin = fs::absolute(env && *env ? env : "scomp");
        if (!fs::exists(bin) || ::access(bin.c_str(), X_OK) != 0) GTEST_SKIP() << "no scomp binary at " << bin;
        scomp = quoted(bin.string());
    }
    ...
    // Runs `command` under /bin/sh with $SCOMP set to the binary; returns the exit status.
    int sh(const std::string& command) const {
        const std::string full = "SCOMP=" + scomp + "; " + command;
...
TEST_F(MainTest, NoArgumentsExit2WithUsageOnStderr) {
    EXPECT_EQ(sh("\"$SCOMP\" >" + quoted(at("out")) + errFile()), 2);
    EXPECT_EQ(errText().rfind("usage: scomp", 0), 0u);
    EXPECT_TRUE(ts::readFile(at("out")).empty());
}

TEST_F(MainTest, HelpExits0WithUsageOnStdout) {
    EXPECT_EQ(sh("\"$SCOMP\" help >" + quoted(at("out")) + errFile()), 0);
    EXPECT_EQ(ts::readText(at("out")).rfind("usage: scomp", 0), 0u);
    EXPECT_TRUE(errText().empty());
}

TEST_F(MainTest, BadUsageExits2) {
    EXPECT_EQ(sh("\"$SCOMP\" frobnicate" + errFile()), 2);
    EXPECT_EQ(errText(), "scomp: unknown command 'frobnicate'\n(scomp help lists the commands and options)\n");
    EXPECT_EQ(sh("\"$SCOMP\" c -o" + errFile()), 2);
    EXPECT_EQ(sh("\"$SCOMP\" t -c x" + errFile()), 2);
}
...
TEST_F(MainTest, ArgumentsReachTheCliUnchanged) {
    ...
    EXPECT_EQ(sh("\"$SCOMP\" c -q " + quoted(in) + errFile()), 0) << errText();
...
TEST_F(MainTest, PipelineRoundTrip) {
    ...
    EXPECT_EQ(sh("cat " + quoted(in) + " | \"$SCOMP\" c | \"$SCOMP\" d >" + quoted(at("out")) + errFile()), 0) << errText();
...
TEST_F(MainTest, PipedArchiveIsExactlyTheLibraryEncoding) {
    ...
    const std::string in = put("in", data);
    EXPECT_EQ(sh("\"$SCOMP\" c <" + quoted(in) + " >" + quoted(at("out")) + errFile()), 0) << errText();
    EXPECT_EQ(ts::readFile(at("out")), encoded(data));
}

TEST_F(MainTest, EmptyStdinGivesTheEmptyArchive) {
    EXPECT_EQ(sh("\"$SCOMP\" c </dev/null >" + quoted(at("out")) + errFile()), 0) << errText();
...
TEST_F(MainTest, FilesRoundTripWithRemoveAndBothDecoders) {
    for (const std::string& decoder : {"", " --bitbybit"}) {
        ...
        ASSERT_EQ(sh("\"$SCOMP\" c -q --rm " + quoted(in) + errFile()), 0) << errText();
        ASSERT_FALSE(fs::exists(in));
        ASSERT_EQ(sh("\"$SCOMP\" d -q --rm" + decoder + " " + quoted(in + ".swas") + errFile()), 0) << errText();
...
TEST_F(MainTest, TestCommandExitStatusFollowsTheArchive) {
    ...
    EXPECT_EQ(sh("\"$SCOMP\" t -q " + quoted(good) + errFile()), 0) << errText();
    EXPECT_EQ(sh("\"$SCOMP\" t -q " + quoted(bad) + errFile()), 1);
    EXPECT_EQ(sh("\"$SCOMP\" t -q " + quoted(good) + " " + quoted(bad) + " " + quoted(good) + errFile()), 1);
}

TEST_F(MainTest, MissingFileExits1) {
    EXPECT_EQ(sh("\"$SCOMP\" c " + quoted(at("ghost")) + errFile()), 1);
    EXPECT_EQ(errText(), "scomp: " + at("ghost") + ": cannot read: No such file or directory\n");
}
...
TEST_F(MainTest, InfoAndBenchRunOnTheBinary) {
    ...
    EXPECT_EQ(sh("\"$SCOMP\" i --codes " + quoted(archive) + " >" + quoted(at("out")) + errFile()), 0) << errText();
    EXPECT_NE(ts::readText(at("out")).find("  crc32     17eaf9b7\n"), std::string::npos);
    EXPECT_EQ(sh("\"$SCOMP\" b -n 1 " + quoted(in) + " >" + quoted(at("out")) + errFile()), 0) << errText();
...
TEST_F(MainTest, DecompressToStdoutConcatenatesInOrder) {
    ...
    EXPECT_EQ(sh("\"$SCOMP\" d -c " + quoted(a) + " " + quoted(b) + " >" + quoted(at("out")) + errFile()), 0) << errText();
...
TEST_P(MainShapeTest, EveryShapeSurvivesThePipe) {
    ...
    const std::string in = put("in", data);
    EXPECT_EQ(sh("\"$SCOMP\" c <" + quoted(in) + " | \"$SCOMP\" d --bitbybit >" + quoted(at("out")) + errFile()), 0) << errText();
    EXPECT_TRUE(ts::readFile(at("out")) == data);
}
