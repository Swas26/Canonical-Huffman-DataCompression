// test_main.cpp -- main(): the built dc binary, run as a separate process.
//
...
// user sees them. The binary is ./dc (make test builds it first) or $DC_BIN;
...
protected:
    ...
    void SetUp() override {
        ASSERT_TRUE(dir.ok());
        const char* env = std::getenv("DC_BIN");
        const fs::path bin = fs::absolute(env && *env ? env : "dc");
        if (!fs::exists(bin) || ::access(bin.c_str(), X_OK) != 0) GTEST_SKIP() << "no dc binary at " << bin;
        dc = quoted(bin.string());
    }
    ...
    // Runs `command` under /bin/sh with $DC set to the binary; returns the exit status.
    int sh(const std::string& command) const {
        const std::string full = "DC=" + dc + "; " + command;
...
TEST_F(MainTest, ArgumentsReachTheCliUnchanged) {
    ...
    EXPECT_EQ(sh("\"$DC\" c -q " + quoted(in) + errFile()), 0) << errText();
...
TEST_F(MainTest, PipelineRoundTrip) {
    ...
    EXPECT_EQ(sh("cat " + quoted(in) + " | \"$DC\" c | \"$DC\" d >" + quoted(at("out")) + errFile()), 0) << errText();
...
TEST_F(MainTest, PipedArchiveIsExactlyTheLibraryEncoding) {
    ...
    const std::string in = put("in", data);
    EXPECT_EQ(sh("\"$DC\" c <" + quoted(in) + " >" + quoted(at("out")) + errFile()), 0) << errText();
    EXPECT_EQ(ts::readFile(at("out")), encoded(data));
}

TEST_F(MainTest, EmptyStdinGivesTheEmptyArchive) {
    EXPECT_EQ(sh("\"$DC\" c </dev/null >" + quoted(at("out")) + errFile()), 0) << errText();
...
TEST_F(MainTest, FilesRoundTripWithRemoveAndBothDecoders) {
    for (const std::string& decoder : {"", " --bitbybit"}) {
        ...
        ASSERT_EQ(sh("\"$DC\" c -q --rm " + quoted(in) + errFile()), 0) << errText();
        ASSERT_FALSE(fs::exists(in));
        ASSERT_EQ(sh("\"$DC\" d -q --rm" + decoder + " " + quoted(in + ".swas") + errFile()), 0) << errText();
...
TEST_F(MainTest, TestCommandExitStatusFollowsTheArchive) {
    ...
    EXPECT_EQ(sh("\"$DC\" t -q " + quoted(good) + errFile()), 0) << errText();
    EXPECT_EQ(sh("\"$DC\" t -q " + quoted(bad) + errFile()), 1);
    EXPECT_EQ(sh("\"$DC\" t -q " + quoted(good) + " " + quoted(bad) + " " + quoted(good) + errFile()), 1);
}

TEST_F(MainTest, MissingFileExits1) {
    EXPECT_EQ(sh("\"$DC\" c " + quoted(at("ghost")) + errFile()), 1);
...
TEST_F(MainTest, InfoAndBenchRunOnTheBinary) {
    ...
    EXPECT_EQ(sh("\"$DC\" i --codes " + quoted(archive) + " >" + quoted(at("out")) + errFile()), 0) << errText();
    EXPECT_NE(ts::readText(at("out")).find("  crc32     17eaf9b7\n"), std::string::npos);
    EXPECT_EQ(sh("\"$DC\" b -n 1 " + quoted(in) + " >" + quoted(at("out")) + errFile()), 0) << errText();
...
TEST_F(MainTest, DecompressToStdoutConcatenatesInOrder) {
    ...
    EXPECT_EQ(sh("\"$DC\" d -c " + quoted(a) + " " + quoted(b) + " >" + quoted(at("out")) + errFile()), 0) << errText();
...
TEST_P(MainShapeTest, EveryShapeSurvivesThePipe) {
    ...
    const std::string in = put("in", data);
    EXPECT_EQ(sh("\"$DC\" c <" + quoted(in) + " | \"$DC\" d --bitbybit >" + quoted(at("out")) + errFile()), 0) << errText();
    EXPECT_TRUE(ts::readFile(at("out")) == data);
}
