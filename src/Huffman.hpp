...
namespace hf{
    ...
    /* building canonical codes off of code lengths, returns false and leaves codes untuched when len is not usable code table

    of if its an all 0 table ie nothing to encode*/

    bool buildCanonicalCodes( const CodeLengths& len, CodeTable& codes); 
}